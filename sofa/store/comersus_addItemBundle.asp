...
Do while request("idProduct"&pCount) <> ""
	' check Stock
	pIdProduct	 	= getUserInput(request("idProduct"&pCount),10)
	p1StepCheckout	 	= getUserInput(request("1StepCheckout"),3)
	pQuantity	 	= getUserInput(request("quantity"&pCount),10)
...
Do while request("idProduct"&pCount) <> ""
	...
	if pQuantity > 0 Then
		...
		' if the product has optionals, is configured to reject and the user has not selected all optionals => send to message
		...
		call customerTracking("comersus_addItemBundle.asp", request.form)
		...
		if pIdCartRow=0 then
		 ...
		 for f=0 to pHowManyOptionalsSelected-1
		  ...
		  ' get the price and optionDescrip (for historic purposes is also saved)
		   mySQL="SELECT priceToAdd, optionDescrip FROM options WHERE idOption="&arrayIdOptions(f)
		   call getFromDatabase(mySQL, rstemp, "addItem")      
