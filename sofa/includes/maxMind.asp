...
<%
	...
	class CreditCardFraudDetection
		...
		public function querySingleServer(serverString)
			...
			if pXmlHTTPComponent="Msxml2.ServerXMLHTTP.5.0" then
			 set objxmlhttp = server.Createobject("Msxml2.ServerXMLHTTP.5.0")
			end if
	...
	Function MaxMindApproved(pIdCustomer, pUserIp, byRef pScore)
	
		dim ccfs, mxObj
		set ccfs 	= new CreditCardFraudDetection
		set mxObj 	= CreateObject("Scripting.Dictionary")
		
		'get user information
		mySQL="SELECT * FROM customers WHERE idCustomer=" &pIdCustomer
		call getFromDatabase(mySQL, rstemp, "CreditCardFrauDetection.class.asp")
		...
		mxObj.Add pMaxMindLicenseKey, "" 		' set the license key
		mxObj.Add "i",		pUserIp				' set the client ip address
		mxObj.Add "city",	pCity				' set the billing city
		mxObj.Add "region", pState				' set the billing state
		mxObj.Add "postal", pZip				' set the billing zip code
		mxObj.Add "country",pCountryCode		' set the billing country
		...
		if ret.item("score") = "" then
			pScore = 10
		else
			pScore = ret.item("score")
		end if
		if pScore <= pMaxMindScoreApproved then
			' it is ok
			MaxMindApproved = -1		
		else
			if pScore <= pMaxMindScoreAlert then
				' it is ok, send mail
	 			call sendmail (pCompany, pEmailSender, pEmailAdmin, "Fraud detection", "The order: #"&pIdorder & " has obtained the score:" & pScore)
				MaxMindApproved = -1		
			else
				' rejected
				MaxMindApproved = 0		
